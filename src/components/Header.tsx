...
import { GradeConfirmModal } from './GradeConfirmModal';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const handleGradeClick = (g: number) => {
    if (g === profile.grade) return;
    setPendingGrade(g);
  };

  const handleConfirmGrade = () => {
    if (pendingGrade && onChangeGrade) {
      onChangeGrade(pendingGrade);
    }
    ...
    <>
      <header className="sticky top-0 z-30 w-full bg-white/95 backdrop-blur-md border-b border-slate-100 shadow-[0_2px_12px_rgba(36,50,74,0.03)] px-4 sm:px-6 lg:px-8 py-3.5 transition-all">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex flex-col">
              <div className="flex items-center gap-2">
                ...
                <span className="ml-1 text-xs font-black text-blue-600 bg-blue-50 px-2.5 py-0.5 rounded-full border border-blue-100">
          ...
          {/* Center: Grade Switcher for Primary School with Confirmation */}
          {onChangeGrade && (
